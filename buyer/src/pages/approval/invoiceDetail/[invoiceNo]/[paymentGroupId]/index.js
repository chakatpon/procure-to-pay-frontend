...
const paymentGroupId = (props) => {
    ...
    if (router.isReady == true) {
      if (["Buyer_Audit_Approver"].includes(get(userData, "role"))) {
        apiConfig = {
          ...
          viewModel: "/p2p/api/v1/template/invoice-to-pay/waitingApproval/detailView",
        };
      } else if (["Buyer_Finance_Maker_Approver", "Buyer_Finance_Maker"].includes(get(userData, "role"))) {
        apiConfig = {
          dataSource: "/p2p/api/v1/view/finance/invoice-to-pay/detail",
          viewModel: "/p2p/api/v1/template/finance/invoice-to-pay/detailView",
        };
      } else {
  ...
  const loadRejectReason = async () => {
    const rejectReasonApi = await AppApi.getApi(
      "/p2p/api/v1/reject/invoice-to-pay/reasonCode",
      {},
      {
        method: "post",
        authorized: true,
  ...
  const resolveCallback = (action) => {
    return get(
      filter(get(viewModel, "footerAction"), (btn) => get(btn, "action") === action),
      [0, "callback"],
      false
    );
  };
  ...
  return (
    <>
      <Head>
        ...
        <link rel="stylesheet" href="/assets/css/pages/waitinginvoicetopayapprovaldetail/waitinginvoicetopayapprovaldetail.css"></link>
      </Head>
      <div className="container">
        ...
        >
          Please confirm to approve Invoice to Pay{" "}
        </DialogConfirm>
        <DialogConfirm
