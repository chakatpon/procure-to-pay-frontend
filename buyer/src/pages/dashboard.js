...
export default function Dashboard(props) {
  ...
  const getSupplierFilter = async() => {
    try {
      ...
      if(get(response, "status") === 200){
        ...
        list = [{supplierCode: "", supplierNameTH: "All"}, ...list]
  ...
  const PopoverSupplier = () => (
    <div style={{width: "300px"}}>
      <p className="bold">Supplier Filter</p>
      <div id="bbl_content" style={{backgroundColor: "#fff"}}>
        <div className="form">
          <div className="control-group">
            <label className="control-label">Showroom</label>
            <div className="controls">
              <Select placeholder="Showroom" style={{ width: '100%' }} onChange={(value) => setFilterValue({...filterValue, supplierCode: value})} defaultValue={""} value={get(filterValue, "supplierCode")}>
                {map(supplierFilter, (item, i) => (
                  <Select.Option key={i} value={get(item, "supplierCode")}>
                    {get(item, "supplierNameTH")}
                  </Select.Option>
                ))}
              </Select>
            </div>
