...
const fundManagement = (props) => {
  ...
  const getData = async (page, pageSize, sortList, searchList, limitOptions, flagShowTotalSum) => {
    ...
    if (list.status == 200) {
      ...
      // ------------- set value of time to format dateTime ---------------
      // const searchListNewMap = searchList.map((search) => {
      //   if (search.field == "threeWayMatchTime") {
      //     const oldVal = search.value;
      //     search.value = threeWayMatchTime || oldVal;
      //   }
      //   return search;
      // });

      // console.log("searchListNewMap : ", searchListNewMap);

      // setSearchList(searchListNewMap);
      // ------------- End of set value of time to format dateTime ---------------
    ...
    try {
      ...
      get(listView.data, "table", []).forEach((table) => {
        ...
        if (table.tableClass == "detailTable") {
          ...
          // ---------- Process for flag to show total amount of forecast amount ----------------
          const colFilterForecastDetails = get(table, "columns")
            .filter((colDetail) => colDetail.columnFieldName == "additionalReference1")
            // .map((col) => {
      ...
      // }
      // const colFilterForecastDetails = colDetail.filter((colDetail) => colDetail.key == "additionalReference1");
