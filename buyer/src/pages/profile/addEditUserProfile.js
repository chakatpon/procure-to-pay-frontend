...
export default function addEditUserProfile() {
  ...
  const [croppedImagUrl, setCroppedImagUrl] = useState(null);
  ...
  const initialData = async () => {
    ...
    if (companyAccessResult) {
      ...
      setTotalItem(companyAccessResult.totalItem);
    ...
    if (userId !== "" && modePage == "edit") {
      ...
      if (userDetailResult) {
        ...
        const buyerCode = _.get(userDetailResult, "companyCode");
  ...
  const getUserDetail = async (id) => {
    try {
      ...
      if (getUserDetail && getUserDetail.status === 200) {
        ...
        // setErrorMessage(_.get(getUserDetail.data, "message", "Something went wrong."));
  ...
  const onGroupCompanyCode = (companyAccessList) => {
    const buyerCodeList = [];
    // ------------- process for group company code and company name to show buyer code list ----------
    _.chain(companyAccessList)
      ...
      .map((value, key) => {
        const buyerGroup = _.chain(value)
          ...
          .value();
        buyerCodeList.push(buyerGroup[0]);
      })
      .value();
  ...
  const propsUpload = {
    ...
    onChange: ({ file, fileList }) => {
      if (fileList.length > 0) {
        // setSrcForCrop(fileList[0].thumbUrl);
  ...
  const onSelectBuyerCode = async (buyerCode, branchCode = "", companyListInit = [], companyListDetail = []) => {
    ...
    // ------- filter company access list from buyer code -------
    companyLists = companyLists.filter((data) => data.companyCode == buyerCode).map((val, index) => ({ ...val, no: index + 1 }));
  ...
  const onSearchCompanyAcc = () => {
    ...
    setTotalRecord(dataCompanyAccess.length);
    setCurrent(1);
  };
  ...
  const onFinish = async () => {
    ...
    const data = {
      ...
      buyerCode: _.get(initialDataForm, "buyerCode", ""),
    ...
    if (id !== "" && mode === "edit") {
      if (statusCode == "PFB11") {
        ...
        try {
          ...
          if (editUserProposal && editUserProposal.status == 200) {
            ...
            // setShowErrorCard(true);
            // setErrorMessage(_.get(editUserProposal.data, "message", ""));
            showAlertDialog({
        ...
        try {
          ...
          if (editUserRes && editUserRes.status == 200) {
            ...
            // setShowErrorCard(true);
            // setErrorMessage(_.get(editUserRes.data, "message", ""));
            showAlertDialog({
      ...
      try {
        ...
        if (createUserRes && createUserRes.status == 200) {
          ...
          // setShowErrorCard(true);
          // setErrorMessage(_.get(createUserRes.data, "message", ""));
          showAlertDialog({
        ...
        <div>
          ...
          >
            <div className="row justify-content-between">
              ...
              {mode == "edit" && initialDataForm.username !== "" && statusCode !== "PFB11" && (
                <Form.Item
                  label="User Name"
                  name="username"
                  rules={[
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Please select Buyer Code",
                  },
                ]}
                style={{ width: "49%" }}
              >
                <Select
                  ...
                  // value={buyerCode}
                  onChange={(value) => {
                    onSelectBuyerCode(value);
                    setInitialDataForm({ ...initialDataForm, buyerCode: value });
                    // setBuyerCode(value);
                  }}
                >
