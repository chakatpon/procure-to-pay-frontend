...
const DialogReason = (props) => {
  ...
  >
    ...
    {get(props,'title',false)?<h5 className="mb-10"><b>{get(props,'title',"")}</b></h5>:<></>}
        <Form.Item name="code" required={false}  placeholder={`Select Reason`} rules={[{required : true, message: "'Reason' is required" }]} label={<>{`${get(props, 'mode', '')} Reason`} <span className="red">*</span></>}>
        ...
        <Form.Item rules={[{required : true, message: "'Note' is required" }]} required={false} name="note" label={<>Note {get(props, "secondStarOn", true) ? <span className="red">*</span> : ""}</>}>
          <Input />
        </Form.Item>
    </Form>
  </Modal></>
