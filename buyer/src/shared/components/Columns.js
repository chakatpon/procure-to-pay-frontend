...
const Columns = (listView,context,AppApi,reloadList) => {
  ...
  const reviseDate = async (config,date,row,event) => {
    try{
      ...
      showLoading("Saving")
      let params = assignParams(row,config,moment(date).format("DD-MM-YYYY"));
  ...
  let col = columnsList.map(r=>{
    ...
    if(get(r,'columnType',"text")=="datepicker"){
      ...
      res = {...res,
        ...
        render: (text,row) => (
          ...
          value={moment(text, get(res,"dateFormat","DD-MM-YYYY"))}
          format={get(res,"dateFormat","DD-MM-YYYY")}
          /></>
        )
      }
    }
    ...
    if(get(r,'columnType',"text")=="number"){
      res = {...res,
            ...
            render: text => (
              ...
              :
              <>{(text || text == 0) ? text.toFixed(get(r,"columnDecimalLength",0)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'): text != null ? text.toFixed(2) : text}</>
            )
       }
    }
    if(get(r,'columnType',"text")=="amount"){
      res = {...res,
            ...
            render: text => {
              ...
              :
              <span>
                {parseFloat(text) ? text.toFixed(get(r,"columnDecimalLength",2)).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'): text != null ? text.toFixed(2) : text}
              </span>
              )
            }
       }
    ...
    if(get(r,'columnType',"text")=="render"){
      res = {...res,
            className : get(r,"columnClassName",get(res,"className","")),
            align : "center",
            render: get(r,"render",get(res,"render",""))
       }
    }
    return res;
  }).filter(r => r !== false);
