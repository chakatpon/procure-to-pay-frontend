import _, { get, isEmpty, isNaN, isNumber, size } from "lodash";
import { Form, Input, Button, Checkbox, Select, DatePicker, TimePicker, Space, Spin, InputNumber } from "antd";
  ...
  useEffect(() => {
    ...
    get(props, "search", []).map((r) => {
      ...
      if (get(r, "searchInputDefaultValue") !== null) {
        if (get(r, "searchChild", {})) {
          if (get(r, "searchChild.searchInputDefaultValue", null) !== null) {
            searchInputDefaultValue = { ...searchInputDefaultValue, [r.searchChild.searchKey]: get(r, "searchChild.searchInputDefaultValue") };
          }

          if (get(r, "searchChild.searchInputDefaultValueOption", null) !== null) {
            searchInputDefaultValue = { ...searchInputDefaultValue, [r.searchChild.searchKey]: get(r, "searchChild.searchInputDefaultValueOption") };
          }
        }
        searchInputDefaultValue = { ...searchInputDefaultValue, [r.searchKey]: get(r, "searchInputDefaultValue") };
      } else if (get(r, "searchInputDefaultValueOption") !== null) {
        if (get(r, "searchChild", {})) {
          if (get(r, "searchChild.searchInputDefaultValue", null) !== null) {
            searchInputDefaultValue = { ...searchInputDefaultValue, [r.searchChild.searchKey]: get(r, "searchChild.searchInputDefaultValue") };
          }
        ...
        searchInputDefaultValue = { ...searchInputDefaultValue, [r.searchKey]: get(r, "searchInputDefaultValueOption") };
      } else {
  ...
  return (
    <Form.Item>
      <Input.Group compact>
        ...
        <Form.Item name={[get(opt, "searchKey", "Untitled"), "end"]}>
          <Input
            disabled={get(opt, "searchDisable", false) !== null ? get(opt, "searchDisable", false) : false}
            // formatter={(value) =>{return isNaN(parseFloat(value)) ? '' : parseFloat(value) ? parseFloat(`${value}`).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") : '.00'}}
...
const TypeMultiSelectRelated = ({ opt, form, onChangeSelect }) => {
  ...
  return (
    <>
      {/* // ----------- main selection ---------------- */}
      <Form.Item form={form} name={get(opt, "searchKey", "Untitled")} rules={!isEmpty(rulesSelect) ? [rulesSelect] : [{}]}>
        <Select
          // mode="multiple"
