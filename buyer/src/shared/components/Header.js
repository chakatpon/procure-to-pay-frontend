...
function Header(props) {
  ...
  const { isLogin, appMenu, setAppMenu, showAlertDialog, showLoading, hideLoading, forceLogin, forceChangePassword, appMenuActive, loginUser, setProfileCanvas, getStorage, setLoginUser, appSocketEndPoint } =
  ...
  const onRedirectNoti = async (notification) => {
    // console.log("noti Redirect : ", notification); // console.log("AAAAA", get(notification, "redirectCode", {}));
    // console.log("AAAAA", get(notification, "redirectParam.id", {}));

    // -------** set id on context **-------
    if (get(notification, "redirectCode", {}) == "PF_BUYER_WAITING_APPROVAL") {
      await context.setBuyerApprovalDetailId(get(notification, "redirectParam.buyerCode", {}));
    }
    if (get(notification, "redirectCode", {}) == "PF_BRANCH_APPROVED") {
      await context.setBranchDetailId(get(notification, "redirectParam.buyerBranchCode", {}));
      await context.setBuyerDetailId(get(notification, "redirectParam.buyerCode", {}));
    }
    if (get(notification, "redirectCode", {}) == "PF_BRANCH_REJECTED" || get(notification, "redirectCode", {}) == "PF_BRANCH_WAITING_APPROVAL") {
      await context.setBranchDetailId(get(notification, "redirectParam.id", {}));
    }
