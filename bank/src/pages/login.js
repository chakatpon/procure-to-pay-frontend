...
export default function Login(props) {
  ...
  const { setProfileCanvas, showLoading, hideLoading, showAlertDialog, setAppMenu, appMenu, getStorage, setStorage, loginUser, setLoginUser, isLogin, setIsLogin, forceChangePassword } =
  ...
  return (
    <>
      <Head>
        <title>LOGIN TO BANK NODE : BBL PROCURE TO PAY</title>
        <link href="assets/css/pages/login/classic/login-4.css" rel="stylesheet" type="text/css" />
      </Head>
      <div
        className="login login-4 login-signin-on d-flex flex-wrap align-items-center"
        id="bbl_login"
      >
        <div className="col-8 mx-auto d-flex flex-wrap login-form p-0 position-relative overflow-hidden">
          ...
          <div id="formContent" className="col-6 login-signin p-8 d-flex">
            <div className="col-12 px-0 align-self-center">
              ...
              <h3 className="mb-4 font-weight-bolder text-center">{t("Login to Bank")}</h3>
              <Form
                labelCol={{ span: 12 }}
                ...
                <Form.Item rowGap={0} label="Password" name="password" className="mb-5" rules={[{ required: true, message: "Please fill in password" }]}>
                  <Input.Password
                    ...
                    iconRender={(passwdVisible) => (passwdVisible ? <EyeTwoTone /> : <EyeInvisibleOutlined />)}
                    autoComplete="off"
