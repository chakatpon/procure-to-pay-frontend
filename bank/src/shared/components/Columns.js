...
const Columns = (listView,context,AppApi) => {
    ...
    if(get(r,'columnType',"text")=="datepicker"){
      ...
      res = {...res,
        ...
        render: (text,row) => (
          ...
          ranges={[]}
          disabledDate={date => {
            return moment().subtract(1, 'days').isSameOrAfter(moment(date))
          }}
          cleanable={false}
          onOk={(date,event)=>{
    ...
    if(get(r,'columnType',"text")=="number"){
      res = {...res,
            ...
            render: text => (
              ...
              :
              <>{(text || text == 0) ? text.toFixed(get(r,"columnDecimalLength",0)).replace(/\d(?=(\d{3})+\.)/g, '$&,') : text != null ? text.toFixed(2) : text}</>
            )
       }
    }
    if(get(r,'columnType',"text")=="amount"){
      res = {...res,
            ...
            render: text => {
              ...
              :
              <span>
                {(parseFloat(text) || text == 0) ? text.toFixed(get(r,"columnDecimalLength",2)).replace(/\d(?=(\d{3})+\.)/g, '$&,') : text != null ? text.toFixed(2) : text}
              </span>
              )
            }
       }
