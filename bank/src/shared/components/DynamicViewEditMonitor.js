...
import { Row, Col, Breadcrumb, Descriptions,Table,Radio,Form,Select,Menu, Dropdown, Button as AntButton, Space } from "antd";
...
const DynamicViewEditMonitor = (props) => {
  ...
  useEffect(()=>{
    ...
    console.log('useEffect-DynamicViewEditMonitor',get(props,"dataDetail",[]))
  },[props])
  ...
  return DynamicViewEditMonitor?<><Form
  form={form}
  onFinish={( allValues) => {
  ...
  {get(viewModel, "topAction", []).map((a) => {
      ...
      if(get(a,"buttonType")=="dropdown"){
        return <><Dropdown overlay={() => {
          return <Menu >
          {get(a,"dropdown").map(r => {
            let disabled = get(r,"roles") ? Object.keys(get(r,"roles",{})).filter(mcode => isAllow(mcode,get(r,"roles."+mcode,[]))).length == 0:false;
            if(!disabled){
              disabled = get(r,"showOn",false) ? Object.keys(get(r,"showOn",{})).filter(key => {
                return get(r,"showOn")[key].includes(get(dataDetail,key));
              }).length==0:disabled;
            }
            if(!disabled){
              disabled = get(r,"hideOn",false) ? Object.keys(get(r,"hideOn",{})).filter(key => {
                return get(r,"hideOn")[key].includes(get(dataDetail,key));
              }).length!=0:disabled;
            }
            ...
            return <Menu.Item onClick={() => { linkActionOnClick(get(r,"action","") ,get(r, "label", "") , dataDetail , r) }}><span  dangerouslySetInnerHTML={{ __html: get(r, "icon", "") + get(r, "label", "") }} /></Menu.Item>
          })}
          </Menu>
        }} placement="bottomRight">
        <Button className="mx-2" style={{minWidth:"auto"}}><i class="fas fa-ellipsis-v" style={{  display : "table-footer-group" }}></i></Button>
      </Dropdown></>
      }
      return <Button
              ...
              <div className={get(col,"itemClass")}>
                ...
                {get(col, "items",[]).map(itm => {
                    ...
                    if (get(itm,"component") == "link") {
                      if (get(itm, "dataSource")){
                        return <div className={get(itm,"class")}>
                          ...
                          }) : <div><span className={"ml-2 "+get(itm, "labelClass")}>{get(itm, "label")}</span><span className="ml-3 mr-3">:</span><a className={get(itm, "linkClass", "included-bbl pr-2")} onClick={() => { linkActionOnClick(get(itm,"onClick", "GotoLink"), get(dataDetail, get(itm, "dataSource")), dataDetail, itm) }}>{get(dataDetail, get(itm, "dataSource"))}</a></div>
                        }
                        </div>
                      }
                    ...
                    if (get(itm, "component") == "paymentExpan") {
                      console.log('dataDetail PaymentExpan',dataDetail)
                      return <CustomModule.PaymentExpan model={itm} data={get(dataDetail, get(itm, "dataSource"))}/>
                    }

                    if(get(itm,"component")=="table"){
                      return  <Table {...get(itm,"tableOptions",{})} className={get(itm,"tableClass")} columns={get(itm,"columns")} dataSource={ get(dataDetail, get(itm, "dataSource"),[]) }></Table>
                    }
                    ...
                    if (get(itm, "component") == "dataCurrency") {
                      return <div className={"mt-2 "+get(itm, "class")}><span className={"ml-2 "+get(itm,"labelClass")}>{get(itm, "label")}</span><span className={get(itm, "itemClass")}>{valueFormat(get(dataDetail, get(itm, "dataKey")), "currency")} THB</span></div>
                    }
                    ...
                    return <div className={get(itm,"class")} >
                        ...
                        {console.log("DATADETAIL L:",dataDetail)}
                        { get(dataDetail, get(itm, "dataKey"))
                        ? <><span className={"ml-2 "+get(itm ,"labelClass")}>{get(itm,"label")}</span> {
                        (((get(itm, "dataKey") == 'status')
                        ...
                        : <span className="ml-2">{get(itm,"label")}{get(dataDetail, get(itm, "dataKey")) == 0
                        ? "0" : null}</span> }
                      </div>
...
export default DynamicViewEditMonitor;
