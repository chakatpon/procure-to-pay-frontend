...
function getEntryFiles() {
    ...
    // Procure 2 Pay css pages (single page use)
...
function getDemos(pathDemos) {
    ...
    let demos = [];
    args.forEach((arg) => {
      if(arg.match(/^design.*/g)){
        demos.push('design');
      }
      if(arg.match(/^buyer.*/g)){
        demos.push('buyer');
      }
      if(arg.match(/^seller.*/g)){
        demos.push('seller');
      }
      if(arg.match(/^bank.*/g)){
        demos.push('bank');
    ...
    if (demos.length === 0) {
        demos = ['design','buyer','seller','bank'];
        if (args.indexOf('allnodes') !== -1) {
            try {
                // sync reusable source code with demo1 for all other demos
                // demos = fs.readdirSync(pathDemos).filter((file) => {
                //     return !/(^|\/)\.[^\/\.]/g.test(file) && /^demo\d+$/g.test(file) && file !== 'design';
                // });
            } catch (err) {
