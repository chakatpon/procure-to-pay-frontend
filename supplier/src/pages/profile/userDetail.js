    ...
    try {
      if (id.id !== null && id.id !== undefined) {
        ...
        if (getUserDetail && getUserDetail.status === 200) {
          ...
          if (statusCode == "PFS01" || statusCode == "PFS06") {
            getReasonCode();
          }
          setStatus(statusCode);
          setUserDetail(getUserDetail.data);
          const companyAccess = _.get(getUserDetail.data, "companyAccessList", []).map((comp, index) => ({ no: index + 1, ...comp }));
  ...
  const getActionHistory = async (id) => {
    try {
      ...
      if (getActionHistory && getActionHistory.status === 200) {
        ...
        setTotalRecordAction(_.get(getActionHistory.data, "totalItem", 0));
    ...
    <div className="container">
      <div className="row bbl-font mt-3 mb-3">
        <Breadcrumb separator=">">
          ...
          <Breadcrumb.Item className="breadcrumb-item active"> {status == "PFS01" || status == "PFS06" ? "User Profile Approval" : "User Profile"}</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      ...
      <div className="row justify-content-md-center">
        ...
        {(status == "PFS08" || status == "PFS09" || status == "PFS11") && isAllow("P6302", ["EDIT"]) && (
          <Button
            className="btn btn-blue mr-2"
            shape="round"
            onClick={() => {
