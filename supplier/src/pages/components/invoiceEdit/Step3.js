...
import _, { get, isEmpty, forEach, filter, has, join, set } from "lodash";
...
const InvoiceFormStep3 = ({ form, api, selectedData, setSelectedData, setStep, showLoading, hideLoading, forceLogin, showAlertDialog, fileLists, attachmentsDel, setAttachmentDel }) => {
  ...
  var timer;
  useEffect(() => {
    setInvoiceInfo(selectedData);
    // form.setFieldsValue({
    //   supplierCodeName: get(invoiceInfo, "supplierDetail.listSupplier.0.value", ""),
    // });
  }, []);
  const getToNextStep = () => {};
  const getToPrevStep = () => {
  ...
  const submitOnFinish = async () => {
    try {
      ...
      showLoading("Resubmitting Invoice");

      let payload = {
        invNo: get(form.getFieldValue(), 'invNo'),
        invDate: moment(get(form.getFieldValue(), 'invoiceDate'), 'DD-MM-YYYY').format("YYYY-MM-DD"),
        invItemCode: get(form.getFieldValue(), 'listVinNo'),
        supplierCodeName: get(form.getFieldValue(), 'supplierCodeName'),
        message: get(form.getFieldValue(), 'note'),
        actionType: "edit",
        attachmentsDel: join(attachmentsDel, ','),
        attachments: filter(fileLists, (f) => has(f, 'name'))
      }

      const defaultSelectedSupplier = get(filter(get(invoiceInfo, ['supplierDetail', 'listSupplier']), (ls) => ls.selectDefault), [0, 'value'])
      const currentSelectedSupplier = get(form.getFieldValue(), 'supplierCodeName')

      if(defaultSelectedSupplier !== currentSelectedSupplier){
        // add 2 keys to payload obj when change supplier branch name
        set(payload, 'supplierVatBranchCode', get(invoiceInfo, ['supplierDetail', 'supplierVatBranchCode']))
        set(payload, 'supplierTaxId', get(invoiceInfo, ['supplierDetail', 'supplierTaxId']))
      }

      let res = await api.getApi("/p2p/api/v1/submit/inv", payload, {
        method: "post",
        headers: {
  ...
  return (
    <>
      ...
      >
        Please confirm to resubmit this Invoice No.{get(invoiceInfo, "invoice.invNo")}
      </DialogConfirm>
      <Form
        form={form}
        labelCol={24}
