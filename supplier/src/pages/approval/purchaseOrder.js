...
const purchaseOrder = (props) => {
  ...
  const onSearchSubmit = async (values) => {
    ...
    showLoading("Searching Waiting Purchase Order Approval List");
  };
  ...
  const getData = async (page, pageSize, sortList, searchList, limitOptions) => {
    showLoading("Loading Waiting Purchase Order Approval List");
    let list = await AppApi.getApi(
      "/p2p/api/v1/inquiry/po/waitingApproval",
      {
        page: page,
        pageSize: pageSize,
        searchList,
  ...
  const prepareData = async (searchList) => {
    try {
      let col = [];
      let listView = {};
      showLoading("Preparing Waiting Purchase Order Approval List");
      ...
      if (codeLists.status == 200) {
        ...
        listView = await AppApi.getApi(
          "/p2p/api/v1/template/waitingPo/listView",
          {},
          { method: "post", authorized: true }
        );
    ...
    <>
      ...
      <DialogConfirm
        ...
        columnDisplay={{
          ...
          "get" : "/p2p/api/v1/template/waitingPo/displaycolumn/get",
          "set" : "/p2p/api/v1/template/waitingPo/displaycolumn/set",
          "default" : "/p2p/api/v1/template/waitingPo/displaycolumn/default"
        }}
        />
      </div>
    </>
