...
const listViewPage = (props) => {
  ...
  const dialogRejectOnFinish = async (values) => {
    try {
      ...
      let appr = await AppApi.getApi("/p2p/api/v1/reject/inv", {
        code: values.code,
        note: values.note,
        rejectList
  ...
  const approveConfirmOnFinish = async (values) => {
    try {
      ...
      let appr = await AppApi.getApi("/p2p/api/v1/approve/inv", {
        approveList
      }, { method: "post", authorized: true });
      hideLoading();
      if (appr.status == 200) {
  ...
  const onSearchSubmit = async (values) => {
    ...
    showLoading("Searching Waiting Invoice Approval List");
  };
  ...
  const loadReason = async () => {
    const reasonApi = await AppApi.getApi('/p2p/api/v1/reject/inv/reasonCode',
      {}, {
      method: "post", authorized: true,
    })
    if (reasonApi.status == 200) {
      ...
      setReason(reasonApi.data.reasonList.map(r=>{ return { option :r.name , value :r.code }}));
    }
  }

  const getData = async (page, pageSize, sortList, searchList, limitOptions) => {
    showLoading("Loading Waiting Invoice Approval List");
    let list = await AppApi.getApi(
      "/p2p/api/v1/inquiry/waitingForApproval/inv/list",
      {
        page: page,
        pageSize: pageSize,
        searchList,
  ...
  const prepareData = async (searchList) => {
    try {
      let col = [];
      let listView = {};
      showLoading("Preparing Waiting Invoice Approval List");

      listView = await AppApi.getApi(
        "/p2p/api/v1/template/waitingInv/listView",
        {},
        { method: "post", authorized: true }
      );
      ...
      <div className="container-fluid px-0">
        ...
        <DynamicListView
          ...
          columnDisplay={{
            ...
            "get": "/p2p/api/v1/template/waitingInv/displaycolumn/get",
            "set": "/p2p/api/v1/template/waitingInv/displaycolumn/set",
            "default": "/p2p/api/v1/template/waitingInv/displaycolumn/default"
          }}
        />
      </div>
    </>
