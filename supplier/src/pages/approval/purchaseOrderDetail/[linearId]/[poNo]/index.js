...
const purchaseOrderDetail = (props) => {
  ...
  useEffect(() => {
    getData(router.query.linearId,router.query.poNo);
  }, [router]);

  const getData = async (linearId, poNo) => {
    setDataDetail(false);
    showLoading(t("Getting Waiting Purchase Order Approval Details"));
    ...
    let resView = { };
      ...
      resView = await AppApi.getApi("/p2p/api/v1/template/waitingPo/detailView", { }, { method: "post", authorized: true });
    ...
    let res = await AppApi.getApi("/p2p/api/v1/view/waitingPo/detail", { poNo, linearId }, { method: "post", authorized: true });
  ...
  const footerAction = (button) => {
    if(get(button,"action")=="action.back"){
      return router.push("/approval/purchaseOrder")
    }
    ...
    try{
      ...
      if(appr.status==200 && get(appr, "data.status",200) == 200){
        showAlertDialog({
          ...
          routerLink: "/approval/purchaseOrder"
        });
        return;
      }
      showAlertDialog({
