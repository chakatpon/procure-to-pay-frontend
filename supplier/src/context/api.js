...
const B2PAPI = (context) => {
  ...
  const getApi = async (endpoint, payload = [], options = []) => {
    ...
    if(get(opts,"headers.Content-Type")=="multipart/form-data"){
      ...
      for(let k in payload){
        ...
        if(typeof payload[k]=="string"){
          ...
          for(let kk in payload[k]){
            console.log('payload[k][kk]',payload[k][kk])
            formData.append(k, payload[k][kk].originFileObj);
          }
        }
