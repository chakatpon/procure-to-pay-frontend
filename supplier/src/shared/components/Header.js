...
function Header(props) {
  ...
  const onReviewNoti = async (flagUpdate) => {
  ...
  const onRedirectNoti = async (notification) => {
    ...
    // ---------- Redirect to page ------------
    showLoading();
    if (get(notification, "redirectCode", "") == "INV_UPLOAD_FILE") {
      router.push(
        {
          pathname: get(notification, "url", "/monitor/invoiceResult/invoiceResultDetail/" + get(notification, "redirectParam.fileName", {})),
          query: { ...get(notification, "redirectParam", {}) },
        },
        get(notification, "url", "/monitor/invoiceResult/invoiceResultDetail/" + get(notification, "redirectParam.fileName", {}))
      );
    } else {
      router.push(
        {
              ...
              <div className="dropdown">
                ...
                <div className="topbar-item" id="" data-toggle="dropdown" data-offset="10px,0px">
                  <div
                    ...
                    onMouseDown={() => onReviewNoti("REVIEW")}
                  >
                      ...
                      {notifyList.length > 0 ? (
                        ...
                        >
                          {notifyList.map((noti, index) => (
                            // <Link href={{ pathname: noti.url, query: noti.redirectParam}} key={index}>
