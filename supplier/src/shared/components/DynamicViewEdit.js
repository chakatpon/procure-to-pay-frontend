...
const DynamicViewEdit = (props) => {
  ...
  const addKey = get(props,"addKey", false)
  const dataIncomplete = get(props,"dataIncomplete", [])
  ...
  <Col span={16}>
    <Breadcrumb separator=">">
      {get(viewModel, "breadcrumb", []).map((a) => {
        ...
        } else if (get(a, "dataKey", "")) {
          return <Breadcrumb.Item className={get(a, "class", "")}>{get(dataDetail, get(a, "dataKey", ""))}</Breadcrumb.Item>;
        } else {
            ...
            <div className={get(col,"blockClass","d-flex "+get(col,"align","align-items-center"))}>
              ...
              <div className={get(col,"itemClass")}>
                ...
                {get(col, "items",[]).map(itm => {
                          ...
                          <div className="container-fluid pl-0">
                            <div className={"row py-2 "+get(r,"class")}>
                              ...
                              { (get(r,"onNull"))?<>
                                ...
                                <SetOption addKey={addKey} dataIncomplete={dataIncomplete} onLoad={()=>{ setShowSubmitBtn(true) }} config={r} onChange={(value, option)=>{
                                  console.log(option)
                                  let data = dataDetail;
                    ...
                    if (get(itm, "component") == "tableCSV") {
                      return <CustomModule.TableCSV model={itm} data={get(dataDetail, get(itm, "dataSource"), dataDetail)}/>
                    }
                    ...
                    return <div className={get(itm,"class")} >
                        ...
                        ? <><span className={get(itm ,"labelClass")}>{get(itm,"label")}</span> {
...
const SetOption =  ({config, onChange , onLoad, addKey, dataIncomplete}) => {
  ...
  useEffect(async()=>{
    showLoading("Getting "+get(config,'label'));
    setLoading(true)
    let resp = await AppApi.getApi(get(config,"onNull.optionApi"),addKey ? dataIncomplete: {},{
