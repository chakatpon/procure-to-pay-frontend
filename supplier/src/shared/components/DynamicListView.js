...
const DynamicListView = (props) => {
  ...
  <div className="container-fluid">
        ...
        <section id="page-content" className="mt-5 px-10">
          ...
          {get(viewModel, "table.tableHeaderAction", []) ? get(viewModel, "table.tableHeaderAction", []).sort((a, b) => get(a, "seq", 1) - get(b, "seq", 1)).map((r) => {
            ...
            let isNotAllow = get(r,"roles") ? Object.keys(get(r,"roles",{})).filter(mcode => isAllow(mcode,get(r,"roles."+mcode,[]))):false;
              if(!isNotAllow){
                if(get(r, 'actionType') === 'buttonAction'){
                  return (
                    <Button
                      onClick={() => {
            ...
            />:<><span className="mx-3 ml-auto"></span></>}
