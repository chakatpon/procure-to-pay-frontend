...
var KTAppChat = function () {
	...
	var _initAside = function () {
		...
		_chatAsideOffcanvasObj = new KTOffcanvas(_chatAsideEl, {
            ...
            //closeBy: 'bbl_chat_aside_close',
            toggleBy: 'bbl_app_chat_toggle'
        });
	...
	return {
		...
		init: function() {
			// Elements
			_chatAsideEl = KTUtil.getById('bbl_chat_aside');
			_chatContentEl = KTUtil.getById('bbl_chat_content');

			// Init aside and user list
			_initAside();

			// Init inline chat example
			KTLayoutChat.setup(KTUtil.getById('bbl_chat_content'));

			// Trigger click to show popup modal chat on page load
			if (KTUtil.getById('bbl_app_chat_toggle')) {
				setTimeout(function() {
					KTUtil.getById('bbl_app_chat_toggle').click();
				}, 1000);
			}
		}
	};
